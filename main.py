...
FILE_EXTENSIONS = {
    "CSV": ".csv",  # Расширение для CSV файлов (табличные данные с разделителями)
    "TXT": ".txt",  # Расширение для текстовых файлов
    "JSON": ".json"  # Расширение для JSON файлов (структурированные данные)
}
...
JSON_PROCESSING_CONFIG = {
    "input_directory": "INPUT",  # Директория для поиска JSON файлов (относительно корня проекта)
    "output_directory": "OUTPUT",  # Директория для сохранения Excel файлов (относительно корня проекта)
    "file_pattern": "*.json",  # Паттерн для поиска JSON файлов (поддерживает wildcards)
    "create_summary": True,  # Создавать лист SUMMARY с общей сводкой данных
    "create_statistics": True,  # Создавать лист STATISTICS с аналитическими данными
    "apply_styling": True  # Применять цветовое оформление и форматирование к Excel файлу
}

# Названия листов для экспорта Excel
# Словарь с названиями листов, которые будут созданы в Excel файле
EXCEL_SHEET_NAMES = {
    "data": "DATA",  # Основной лист с данными (плоская структура из JSON)
    "summary": "SUMMARY",  # Лист с общей сводкой и метаданными
    "statistics": "STATISTICS"  # Лист со статистическими данными и аналитикой
}
...
EXCEL_COLORS = {
    "header": "366092",  # Темно-синий цвет для заголовков (основной)
    "subheader": "9BC2E6",  # Светло-синий цвет для подзаголовков
    "alternate": "E7E6E6",  # Светло-серый цвет для чередующихся строк
    "highlight": "FFEB9C"  # Желтый цвет для выделения важных данных
}
...
REQUEST_PARAMETERS = {  # Дополнительные параметры запросов (заглушка для будущего использования)
    "param1": "value1",  # Параметр 1 (пример)
    "param2": "value2"   # Параметр 2 (пример)
}
