...
# -*- coding: utf-8 -*-
"""
Индивидуальный парсер для обработки JSON данных REWARD
Автор: OrionFLASH
Описание: Специализированный скрипт для обработки данных о наградах сотрудников.
...
def main():
    """
    Основная функция для обработки данных REWARD
    
    Загружает JSON файл с данными о наградах сотрудников, разворачивает
    ...
    input_file = os.path.join(input_dir, "reward_response.json")  # Входной JSON файл
    output_file = os.path.join(output_dir, "reward_data.xlsx")    # Выходной Excel файл
    
    # Создание процессора с именем "reward" и уровнем логирования DEBUG
    processor = JSONToExcelProcessor("reward", "DEBUG")
    
    # Получение конфигурации для данного типа данных
    # Включает настройки листа, включаемые и исключаемые поля
    config = create_processor_config()["reward"]
